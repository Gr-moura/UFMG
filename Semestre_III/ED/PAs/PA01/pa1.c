    ...
    for (int h = n / 2; h > 0; h /= 2)
    {
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = i + h; j < n; j += h)
            {
                ...
                inccmp(s, 1);
                while (anterior >= 0 && aux < A[anterior])
                {
                    incmove(s, 1);
                    A[anterior + h] = A[anterior];
...
void selectionSort(int arr[], int l, int r, sortperf_t *s)
{
    ...
    // Achar o menor e botar em primeiro
    for (int i = 0; i <= r; i++)
    {
        int menorIndex = i;
        for (int j = i + 1; j <= r; j++)
...
void insertionSort(int v[], int l, int r, sortperf_t *s)
{
    ...
    for (int i = l + 1; i <= r; i++)
    {
        ...
        // Enquanto j estiver a frente de seu lugar original
        inccmp(s, 1);
        while (j >= 0 && aux < v[j])
        {
            // Movemos tudo um para a frente
            incmove(s, 1);
            v[j + 1] = v[j];
            j--;
